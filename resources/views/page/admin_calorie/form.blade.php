<div ng-class="
    {'form-group':true,
        'has-error':errors.user_id.length > 0,
        'has-feedback':errors.user_id.length > 0}">
    <label>User</label>
    <select 
        class="form-control"
        ng-model="model.user_id"
        ng-options="user.id as user.name for user in list_user">
        <option style="display:none" value="">-- Select--</option>
    </select>
    <span 
        ng-if="errors.user_id.length > 0"
        class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>
    <span 
        ng-if="errors.user_id.length > 0">[[errors.user_id]]</span>
</div>
<div ng-class="
    {'form-group':true,
